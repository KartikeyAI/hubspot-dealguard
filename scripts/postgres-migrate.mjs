...
if (!numbers.every((number, index) => index === 0 ? number === 1 : number === numbers[index - 1] + 1)) {
  throw new Error(`PostgreSQL migrations are not contiguous: ${numbers.join(', ')}`);
}
...
try {
  ...
  const appliedRows = tableExists || !checkOnly
    ? (await client.query(`SELECT version, name, checksum FROM dealguard.schema_migrations ORDER BY version`)).rows
    : [];
  ...
  for (const name of files) {
    ...
    if (existing) {
      if (existing.name !== name || existing.checksum !== checksum) {
        throw new Error(`Migration ${version} changed after application. Expected ${existing.name}/${existing.checksum}, found ${name}/${checksum}.`);
      }
  ...
  if (checkOnly) {
    ...
    for (const migration of pending) {
      ...
      try {
        ...
        await client.query(
          `INSERT INTO dealguard.schema_migrations (version, name, checksum) VALUES ($1, $2, $3)`,
          [migration.version, migration.name, migration.checksum],
        );
        if (!dryRun) await client.query('COMMIT');
        console.log(`${dryRun ? 'validated' : 'applied'} ${migration.name}`);
      } catch (error) {
    ...
    if (dryRun) {
      await client.query('ROLLBACK');
      console.log(`Dry-run validated ${pending.length} pending migration(s).`);
    } else {
      console.log(`PostgreSQL migrations complete through ${files.at(-1)}.`);
    }
  }
} finally {
