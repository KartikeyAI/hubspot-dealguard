...
const failures = [];
if (record.schemaVersion !== 2) failures.push('unsupported staging evidence schema');
...
if (record.health?.status !== 'ok' || record.health?.service !== 'dealguard-api') failures.push('staging health identity is invalid');
if (record.health?.version !== expectedVersion) failures.push('staging health version does not match package version');
if (Number(record.smoke?.summary?.failed ?? 1) !== 0) failures.push('staging public smoke was not successful');
if (record.smoke?.expectedVersion !== expectedVersion) failures.push('staging public smoke version does not match package version');
if (record.acceptance?.profile !== 'full') failures.push('staging acceptance profile is not full');
if (Number(record.acceptance?.summary?.failed ?? 1) !== 0) failures.push('staging acceptance was not successful');
if (!record.backupReference) failures.push('staging deployment lacks a backup reference');
