...
export function RecommendationDeliverySloPanel({ enabled }: { enabled: boolean }) {
  ...
  return <Flex direction="column" gap="medium">
    ...
    <Flex direction="column" gap="small">
      ...
      <Input name="delivery-slo-name" label="Objective name" value={form.name} onChange={(value: string) => setForm((current) => ({ ...current, name: value }))} />
      ...
      ].filter((option) => targetCompatible(form.metric, option.value as TargetType))} onChange={(value: string) => setForm((current) => ({ ...current, targetType: value as TargetType, targetId: '' }))} />
      {form.targetType !== 'portal' ? <Select name="delivery-slo-target-id" label="Target record" value={form.targetId} options={targetOptions.slice(1)} onChange={(value: string) => setForm((current) => ({ ...current, targetId: value }))} /> : null}
      ...
      <Select name="delivery-slo-severity" label="Alert severity" value={form.severity} options={[{ label: 'Warning', value: 'warning' }, { label: 'Critical', value: 'critical' }]} onChange={(value: string) => setForm((current) => ({ ...current, severity: value as 'warning' | 'critical' }))} />
      <Select name="delivery-slo-route" label="Portal-wide notification route" value={form.notificationRouteId} options={routeOptions} onChange={(value: string) => setForm((current) => ({ ...current, notificationRouteId: value }))} />
