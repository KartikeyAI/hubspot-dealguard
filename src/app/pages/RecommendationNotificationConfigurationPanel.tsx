...
export function RecommendationNotificationConfigurationPanel({ enabled }: { enabled: boolean }) {
  ...
  return <Flex direction="column" gap="medium">
    ...
    <Flex direction="column" gap="small">
      ...
      <Select name="recommendation-channel-type" label="Channel type" value={channel.type} options={[{ label: 'Slack webhook', value: 'slack_webhook' }, { label: 'Microsoft Teams workflow', value: 'teams_workflow' }, { label: 'Email', value: 'email' }, { label: 'Signed webhook', value: 'webhook' }]} onChange={(value) => setChannel((current) => ({ ...current, type: String(value) as ChannelType }))} />
      <Input name="recommendation-channel-name" label="Channel name" value={channel.name} onChange={(value) => setChannel((current) => ({ ...current, name: String(value) }))} />
      {channel.type === 'email'
        ? <Input name="recommendation-channel-recipients" label="Email recipients" description="Comma-separated operational recipients." value={channel.recipients} onChange={(value) => setChannel((current) => ({ ...current, recipients: String(value) }))} />
        : <Input name="recommendation-channel-endpoint" label={channel.id ? 'Replacement HTTPS endpoint (optional)' : 'HTTPS endpoint'} value={channel.endpoint} onChange={(value) => setChannel((current) => ({ ...current, endpoint: String(value) }))} />}
      {channel.type === 'webhook' ? <Input name="recommendation-channel-signing-secret" label="Signing secret (optional)" description="Leave blank on creation to generate a secret." value={channel.signingSecret} onChange={(value) => setChannel((current) => ({ ...current, signingSecret: String(value) }))} /> : null}
    ...
    <Flex direction="column" gap="small">
      ...
      <Input name="recommendation-calendar-name" label="Calendar name" value={calendar.name} onChange={(value) => setCalendar((current) => ({ ...current, name: String(value) }))} />
      <Input name="recommendation-calendar-timezone" label="IANA timezone" value={calendar.timezone} onChange={(value) => setCalendar((current) => ({ ...current, timezone: String(value) }))} />
      <Flex direction="row" gap="small"><Input name="recommendation-calendar-start" label="Weekday start (HH:MM)" value={calendar.start} onChange={(value) => setCalendar((current) => ({ ...current, start: String(value) }))} /><Input name="recommendation-calendar-end" label="Weekday end (HH:MM)" value={calendar.end} onChange={(value) => setCalendar((current) => ({ ...current, end: String(value) }))} /></Flex>
      <Input name="recommendation-calendar-holidays" label="Holiday dates" description="Comma-separated YYYY-MM-DD dates." value={calendar.holidays} onChange={(value) => setCalendar((current) => ({ ...current, holidays: String(value) }))} />
    ...
    <Flex direction="column" gap="small">
      ...
      <Input name="recommendation-route-name" label="Route name" value={route.name} onChange={(value) => setRoute((current) => ({ ...current, name: String(value) }))} />
      <MultiSelect name="recommendation-route-events" label="Explicit recommendation events" options={EVENT_OPTIONS} value={route.eventTypes} onChange={(value) => setRoute((current) => ({ ...current, eventTypes: value.map(String) }))} />
      <MultiSelect name="recommendation-route-channels" label="Enabled channels" options={channelOptions} value={route.channelIds} onChange={(value) => setRoute((current) => ({ ...current, channelIds: value.map(String) }))} />
      <Flex direction="row" gap="small" wrap="wrap"><Select name="recommendation-route-severity" label="Minimum severity" value={route.minimumSeverity} options={[{ label: 'Info', value: 'info' }, { label: 'Warning', value: 'warning' }, { label: 'Critical', value: 'critical' }]} onChange={(value) => setRoute((current) => ({ ...current, minimumSeverity: String(value) as Severity }))} /><Select name="recommendation-route-calendar" label="Quiet-hours calendar" value={route.quietHoursCalendarId} options={calendarOptions} onChange={(value) => setRoute((current) => ({ ...current, quietHoursCalendarId: String(value) }))} /><NumberInput name="recommendation-route-cooldown" label="Suppression window minutes" value={route.suppressionWindowMinutes} min={0} max={10080} onChange={(value: number) => setRoute((current) => ({ ...current, suppressionWindowMinutes: value }))} /></Flex>
      <Flex direction="row" gap="small" wrap="wrap"><Input name="recommendation-route-pipelines" label="Pipeline IDs" value={route.pipelineIds} onChange={(value) => setRoute((current) => ({ ...current, pipelineIds: String(value) }))} /><Input name="recommendation-route-teams" label="Team IDs" value={route.teamIds} onChange={(value) => setRoute((current) => ({ ...current, teamIds: String(value) }))} /><Input name="recommendation-route-owners" label="Owner IDs" value={route.ownerIds} onChange={(value) => setRoute((current) => ({ ...current, ownerIds: String(value) }))} /><Input name="recommendation-route-regions" label="Region codes" value={route.regionCodes} onChange={(value) => setRoute((current) => ({ ...current, regionCodes: String(value) }))} /></Flex>
