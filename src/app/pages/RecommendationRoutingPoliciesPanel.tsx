...
export function RecommendationRoutingPoliciesPanel({ enabled }: { enabled: boolean }) {
  ...
  return <Flex direction="column" gap="medium">
    ...
    <Flex direction="column" gap="small">
      ...
      <Input name="recommendation-policy-name" label="Policy name" value={form.name} onChange={(value) => { setForm((current) => ({ ...current, name: String(value) })); setPreview(null); }} />
      <Flex direction="row" gap="small" wrap="wrap">
        <Select name="recommendation-policy-trigger" label="Trigger" value={form.trigger} options={[{ label: 'Due soon', value: 'due_soon' }, { label: 'Overdue', value: 'overdue' }]} onChange={(value) => { setForm((current) => ({ ...current, trigger: String(value) as Trigger, routeId: '' })); setPreview(null); }} />
        <Select name="recommendation-policy-lifecycle" label="Lifecycle" value={form.statusScope} options={[{ label: 'Accepted only', value: 'accepted' }, { label: 'Presented only', value: 'presented' }, { label: 'Presented and accepted', value: 'both' }]} onChange={(value) => { setForm((current) => ({ ...current, statusScope: String(value) as StatusScope })); setPreview(null); }} />
        <Select name="recommendation-policy-priority" label="Minimum priority" value={form.minimumPriority} options={[{ label: 'High', value: 'high' }, { label: 'Medium', value: 'medium' }, { label: 'Low', value: 'low' }]} onChange={(value) => { setForm((current) => ({ ...current, minimumPriority: String(value) as Priority })); setPreview(null); }} />
        <Select name="recommendation-policy-severity" label="Severity" value={form.severity} options={[{ label: 'Warning', value: 'warning' }, { label: 'Critical', value: 'critical' }]} onChange={(value) => { setForm((current) => ({ ...current, severity: String(value) as Severity })); setPreview(null); }} />
      </Flex>
      <Flex direction="row" gap="small" wrap="wrap">
      ...
      <Select name="recommendation-policy-route" label={`Initial route · must include ${event}`} value={form.routeId} options={initialOptions} onChange={(value) => { setForm((current) => ({ ...current, routeId: String(value) })); setPreview(null); }} />
      ...
      <Select name="recommendation-policy-escalation-route" label="Manager escalation route" value={form.escalationRouteId} options={escalationOptions} onChange={(value) => { setForm((current) => ({ ...current, escalationRouteId: String(value) })); setPreview(null); }} />
      ...
      <TextArea name="recommendation-policy-note" label="Deterministic notification guidance" description="10–2,000 characters. Do not include contact details or communication content." value={form.managerNote} rows={3} maxLength={2000} resize="vertical" onChange={(value) => { setForm((current) => ({ ...current, managerNote: String(value) })); setPreview(null); }} />
