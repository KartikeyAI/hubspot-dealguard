...
const INTERNAL_PATTERNS = [/relation\s+["'`]?[^^\s"'`]+["'`]?\s+does not exist/i,/column\s+["'`]?[^^\s"'`]+["'`]?\s+does not exist/i,/postgres(?:ql)?/i,/neon/i,/cloudflare/i,/dodo(?:\s+payments)?/i,/sqlstate/i,/syntax error at or near/i,/violates .* constraint/i,/duplicate key value/i,/worker\/src\//i];
export function safeProductError(message: unknown, fallback = 'DealGuard could not complete that action. Please try again.'): string { if (typeof message !== 'string' || !message.trim()) return fallback; return INTERNAL_PATTERNS.some((pattern) => pattern.test(message)) ? fallback : message; }
