...
test('executive revenue endpoint is Enterprise-scoped, read-only and failure-isolates snapshot persistence', () => {
  ...
  const nextRoute = read('worker/src/routes-v13.ts');
  ...
  assert.match(nextRoute, /from '\.\/routes-v12\.js'/);
...
test('documentation and focused CI cover the executive revenue view', () => {
  ...
  assert.match(docs, /available inside HubSpot App Home/);
  assert.match(docs, /intentionally loaded on demand/);
