...
const component = await readFile('src/app/pages/ManagerDecisionQueueCore.tsx', 'utf8');
const composition = await readFile('src/app/pages/ManagerDecisionQueuePanel.tsx', 'utf8');
...
test('App Home panel loads the scoped manager queue with bounded server-side filters', () => {
  ...
  assert.match(composition, /ManagerDecisionQueueCore/);
});
