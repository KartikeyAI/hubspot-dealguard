...
test('release uses direct Neon PostgreSQL and contains no database proxy or D1 runtime', async () => {
  ...
  const releaseOperations = (await Promise.all([
  ...
  assert.doesNotMatch(
    releaseOperations,
    /HYPERDRIVE_CONFIG_ID|Cloudflare Hyperdrive|runtime database traffic must use Hyperdrive|switch Hyperdrive/i,
  );
  assert.match(releaseOperations, /direct TLS-enforced Neon PostgreSQL connectivity/i);
  assert.match(releaseOperations, /Direct Worker-to-Neon connectivity/);
