...
test('App Home SLO controls are on-demand, permission-aware and non-mutating', () => {
  ...
  assert.match(panel, /recommendation-delivery-slos\/routes\/\$\{encodeURIComponent\(routeId\)\}\/enable-events/);
  assert.match(panel, /recommendation-delivery-slos\/incidents\/\$\{encodeURIComponent\(incidentId\)\}\/acknowledge/);
...
test('dedicated documentation, schema validation and CI cover delivery SLO enforcement', () => {
  ...
  assert.match(docs, /operational delivery objective/i);
  assert.match(docs, /insufficient evidence/i);
