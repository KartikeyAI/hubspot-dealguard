...
test('follow-up APIs require Enterprise entitlement, remediation.bulk and human confirmation', () => {
  ...
  assert.match(route, /FOLLOWUP_ROOT = '\/api\/v1\/enterprise\/recommendation-followups'/);
  assert.match(route, /\$\{FOLLOWUP_ROOT\}\/preview/);
  ...
  assert.match(read('worker/src/routes-v15.ts'), /from '\.\/routes-v14\.js'/);
});
...
test('dedicated CI and documentation cover recommendation operations and routing', () => {
  ...
  assert.match(docs, /without adding autonomous CRM changes/i);
