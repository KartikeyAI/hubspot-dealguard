...
function attempt(row: AttemptRow): RecommendationDeliverySloAttempt {
  const results = parseJson<unknown[]>(row.delivery_summary_json, []).flatMap((entry) => {
    if (!entry || typeof entry !== 'object' || Array.isArray(entry)) return [];
    const item = entry as Record<string, unknown>;
    const channelId = typeof item.channelId === 'string' ? item.channelId : null;
    const status = item.status === 'delivered' || item.status === 'failed' ? item.status : null;
    return channelId && status ? [{ channelId, status }] : [];
  });
  return {
