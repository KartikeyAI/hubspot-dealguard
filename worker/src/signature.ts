...
export async function validateHubSpotSignature(request: Request, env: Env): Promise<void> {
  ...
  if (signatureV3 && timestampHeader) {
    ...
    if (!Number.isFinite(timestamp) || Math.abs(Date.now() - timestamp) > MAX_AGE_MS) throw new AppError(401, 'expired_signature', 'HubSpot request timestamp is outside the accepted window.');
    ...
    else if (legacySignature && version === 'v1') valid = constantTimeEqual(await sha256Hex(`${env.HUBSPOT_CLIENT_SECRET}${rawBody}`), legacySignature);
  }
...
export async function validateHubSpotRequest(request: Request, env: Env): Promise<RequestIdentity> {
  ...
  if (!portalId || !/^\d+$/.test(portalId)) throw new AppError(401, 'missing_portal_identity', 'HubSpot portal identity is missing.');
  const appId = url.searchParams.get('appId');
  if (env.HUBSPOT_APP_ID && env.HUBSPOT_APP_ID !== 'REPLACE_WITH_HUBSPOT_APP_ID' && appId && appId !== env.HUBSPOT_APP_ID) throw new AppError(401, 'app_identity_mismatch', 'HubSpot app identity does not match this installation.');
  return { portalId, userId: url.searchParams.get('userId'), userEmail: url.searchParams.get('userEmail'), appId };
}
