...
function hasSlackEntitlement(plan: string): boolean {
  return plan === 'growth' || plan === 'beta_growth';
}

function requireSlackConfiguration(env: Env): void {
  if (!env.SLACK_CLIENT_ID || !env.SLACK_CLIENT_SECRET) {
    throw new AppError(503, 'slack_not_configured', 'Slack integration is not configured for this DealGuard deployment.');
  }
}
...
export async function createSlackAuthorization(env: Env, identity: RequestIdentity): Promise<string> {
  ...
  await env.DB.prepare(
    `INSERT INTO integration_oauth_states (state_hash, provider, portal_id, user_id, user_email, expires_at, created_at)
     VALUES (?, 'slack', ?, ?, ?, ?, ?)`
...
export async function completeSlackAuthorization(env: Env, code: string, state: string): Promise<string> {
  ...
  const oauthState = await env.DB.prepare(
    `SELECT portal_id, user_id, user_email, expires_at FROM integration_oauth_states WHERE state_hash = ? AND provider = 'slack'`
  ...
  if (!response.ok || !payload.ok || !payload.incoming_webhook?.url || !payload.team?.id || !payload.access_token) {
    throw new AppError(502, 'slack_oauth_failed', `Slack could not be connected${payload.error ? `: ${payload.error}` : '.'}`);
  }
...
export async function disconnectSlack(env: Env, identity: RequestIdentity): Promise<void> {
  ...
  if (connection) {
    try {
      ...
      console.error(JSON.stringify({ level: 'warn', task: 'slack_revoke', portalId: identity.portalId, error: error instanceof Error ? error.message : String(error) }));
    }
  }
...
function dealUrl(portalId: string, dealId: string): string {
...
async function reserveNotification(env: Env, portalId: string, dealId: string | null, kind: SlackNotificationKind, fingerprint: string, cooldownMinutes: number): Promise<string | null> {
  ...
  const existing = await env.DB.prepare(`SELECT id, status FROM notification_events WHERE portal_id = ? AND fingerprint = ?`)
    .bind(portalId, fingerprint).first<{ id: string; status: string }>();
  if (existing?.status === 'failed') {
    await env.DB.prepare(`UPDATE notification_events SET status = 'pending', error_message = NULL, sent_at = NULL WHERE id = ?`).bind(existing.id).run();
    return existing.id;
  }
